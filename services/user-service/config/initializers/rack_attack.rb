...
class Rack::Attack
  ...
  ActiveSupport::Notifications.subscribe('rack.attack') do |name, start, finish, request_id, payload|
    ...
    Rails.logger.warn "Rack::Attack: #{payload[:match_discriminator]} - IP: #{req.ip}, Path: #{req.path}, User-Agent: #{req.user_agent}"
    
    # You could also send this to your monitoring service
    # Sentry.capture_message("Rack::Attack: #{payload[:match_discriminator]}", level: :warning) if defined?(Sentry)
    
    # Log additional context for security analysis
    if payload[:match_discriminator].include?('blocklist')
      Rails.logger.warn "Rack::Attack: Security threat detected - #{payload[:match_discriminator]}"
    end
  end
end 